...
const GSTForm = ({ onSubmit }) => {
    ...
    useEffect(() => {
        const fetchHotelAndGSTPercentage = async () => {
            try {
                const hotelsResponse = await axios.get('https://vercelbackend-ashy.vercel.app/api/hotel/get-all');
    ...
    useEffect(() => {
        const fetchGSTPercentage = async () => {
            try {
                if (hotel._id) {
                    const gstResponse = await axios.get(`https://vercelbackend-ashy.vercel.app/api/hotel/get/${hotel._id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        if (hotel._id && gstPercentage !== '') {
            try {
                await axios.patch(`https://vercelbackend-ashy.vercel.app/api/hotel/gst/${hotel._id}`, {
